// adapted from https://gist.github.com/cahilfoley/4b1b2f3fa9e2f9652ee1d8501443b5ca
...
export class Semaphore {
    ...
    acquire = (): Promise<Lock> => {
        dlog(
            `semaphore: lock requested for the ${this.label} resource - ${this.running} active, ${this.waiting.length} waiting`,
        );
        ...
        dlog(
            `semaphore: max active locks hit for the ${this.label} resource - there are ${this.running} tasks running and ${this.waiting.length} waiting.`,
        );
    ...
    purge = () => {
        dlog(
            `semaphore: purge requested on the ${this.label} semaphore, ${this.waiting.length} pending tasks will be cancelled.`,
        );
